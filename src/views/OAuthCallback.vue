...
onMounted(async () => {
  ...
  try {
    ...
    const companyModes =
      response.access_modes?.filter((mode: { type: string }) => mode.type === 'company') || []

    // Map to Company interface
    const companies = companyModes.map(
      (mode: { empresa: unknown; rol: string; empresa_id: number }) => ({
        ...mode.empresa,
  ...
  <div class="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900">
    <div class="max-w-md w-full bg-white dark:bg-gray-800 rounded-lg shadow-md p-8">
      ...
      <div v-if="error" class="text-center">
        <div
          class="mx-auto mb-4 w-16 h-16 flex items-center justify-center rounded-full bg-red-100"
        >
          <svg
      ...
      <div v-else-if="processing" class="text-center">
        ...
        <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-4">Autenticando...</h2>
        <p class="text-gray-600 dark:text-gray-400">
          Por favor espera mientras procesamos tu autenticación.
        </p>
        <p class="mt-4 text-sm text-gray-500 dark:text-gray-500">Esto solo tomará unos segundos.</p>
      </div>
    </div>
  </div>
</template>
